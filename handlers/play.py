...
async def playlist(client, message):
    ...
    msg = "****الاغنيه قيد التشغيل حاليا** {}".format(message.chat.title)
    msg += "\n• "+ now_playing
...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        stats = "اعدادات **{}**".format(chat.title)
        if len(que) > 0:
            stats += "\n\n"
            stats += "الحــجم : {}%\n".format(vol)
    ...
    mar = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("⏸", "تشغيل"),
                InlineKeyboardButton("▶️", "ايقاف مؤقت"),
                InlineKeyboardButton("⏭", "تغطي")
            ],
            [
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "انضممت الى هنا لتشغيل الموسيقى ، استمتعو ♪  "
                    )
                    await lel.edit(
                        "<b>لقد انضم الى الدردشة الخاصه بك</b>",
                    )
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("📣 ᴄʜᴀɴɴᴇʟ", url=f"https://t.me/XkkkU")
                ],
            ]
        )
        requested_by = message.from_user.first_name
